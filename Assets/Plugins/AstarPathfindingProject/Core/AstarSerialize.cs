...
namespace Pathfinding {
	public class AstarSerializer {
		...
		public int deserializingGraphIndex = 0;
		...
		public void SetUpGraphRefs (NavGraph[] graphs) {
			...
			for (int i=0;i<loadedGraphGuids.Length;i++) {
				...
				Guid guid = new Guid (loadedGraphGuids[i]);
		...
		public void SerializeNodes (NavGraph graph, AstarPath active) {
			if (mask == SMask.SaveNodes) {
				...
				if (mask == SMask.RunLengthEncoding) {
					...
					//Penalties
					int lastValue = graph.nodes[0].penalty;
					int lastEntry = 0;
					...
					for (int i=1;i<graph.nodes.Length;i++) {
						if (graph.nodes[i].penalty != lastValue || (i-lastEntry) >= byte.MaxValue-1) {
							...
							lastValue = graph.nodes[i].penalty;
							lastEntry = i;
						}
					}
		...
		public virtual UserConnection[] DeserializeUserConnections () {
			...
			if (MoveToAnchor ("UserConnections")) {
				...
				for (int i=0;i<count;i++) {
					...
					conn.overridePenalty = stream.ReadInt32 ();
		...
		private void SerializeNode (Node node, BinaryWriter stream) {
			...
			if (mask == SMask.SaveNodePositions) {
				...
				stream.Write (node.position.z);
			}
			
			if (mask != SMask.RunLengthEncoding) {
				stream.Write (node.penalty);
			...
			if (mask == SMask.SaveNodeConnections) {
				if (node.connections != null) {
					...
					if (node.connections.Length >= 256) {
						Debug.LogError ("Node connections over 256 (one byte), can't serialize");
						WriteError ();
						return;
					}
					
					stream.Write ((byte)node.connections.Length);
				
					for (int i=0;i<node.connections.Length;i++) {
						//The index value for each node is stored in the G variable (a bit dirty, but it saves memory)
						//H value = Graph index
						//G value = Node index in graph
						
						//Graph index as in, which graph
						int graphIndex = node.connections[i].h;
						
						int nodeIndex = node.connections[i].g;
						
						//Pack two numbers into a single 32 bit integer
						stream.Write ((nodeIndex & 0x3FFFFFF) | ((graphIndex & 0x3F) << 26));
						
						if (mask == SMask.SaveNodeConnectionCosts) {
							stream.Write (node.connectionCosts[i]);
						}
						
						//If the connection node is placed in a different graph
						if (graphIndex != node.h) {
							Node other = node.connections[i];
							bool containsLink = false;
							for (int q=0;q<other.connections.Length;q++) {
								if (other.connections[q] == node) {
									containsLink = true;
									break;
								}
							}
							
							stream.Write (containsLink ? true : false);
						}
					}
				} else {
					stream.Write ((byte)0);
		...
		private void DeSerializeNode (Node node, NavGraph[] graphs, int graphIndex, BinaryReader stream) {
			...
			if (mask == SMask.SaveNodeConnections) {
				...
				for (int i=0;i<numConn;i++) {
					...
					/** \todo Don't use deserializingGraphIndex */
					if (nodeGraphIndex != graphIndex) {
						containsLink = stream.ReadBoolean ();
					}
	
					if (graphRefGuids[graphIndex] != -1) {
						Node other = active.astarData.GetNode (graphRefGuids[graphIndex],nodeIndex, graphs);
